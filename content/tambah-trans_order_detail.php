...
if (isset($_POST['id_order'])) {
   ...
   $updateQ = mysqli_query($config, "UPDATE trans_order_detail 
    SET id_order='$id_order', id_service='$id_service', qty='$qty', subtotal='$subtotal'
    WHERE id='$editOrderDetail'");

    if($updateQ){
        header("location: ?page=trans_order_detail&id=$id_orderDetail&update=berhasil");
    } 
}
                    ...
                    <div class="mb-3">
                        ...
                        <select name="id_order" class="form-control" required>
                            ...
                            <option value="<?= $order['id'] ?>"
                                <?= (isset($rowEdit['id_order']) && $rowEdit['id_order'] == $order['id']) ? 'selected' : '' ?>>
                                <?= $order['order_code'] ?>
                            </option>
                            <?php endforeach; ?>
                        </select>
